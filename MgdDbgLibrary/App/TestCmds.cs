    ...
    public class TestCmds
    {
        ...
        public void TestAngle()
        {
            ...
            ed.WriteMessage(string.Format("\nAngle 1: {0}",
                Autodesk.AutoCAD.Runtime.Converter.AngleToString(angle1, AngularUnitFormat.Current, -1)));
            ed.WriteMessage(string.Format("\nAngle 2: {0}",
                Autodesk.AutoCAD.Runtime.Converter.AngleToString(angle2, AngularUnitFormat.Current, -1)));
        }

        [CommandMethod("MgdDbgGetImageAngle", CommandFlags.Modal)]
        public void TestImageAngle()
        {
            Editor ed = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor;
            ...
            if (prRes.Status == PromptStatus.OK)
            {
                ...
                using (Autodesk.AutoCAD.DatabaseServices.Transaction tr = tm.StartTransaction())
                {
                    ...
                    ed.WriteMessage(string.Format("\nECS rotation angle: {0}",
                        Autodesk.AutoCAD.Runtime.Converter.AngleToString(rotAngle1, AngularUnitFormat.Current, -1)));

                    Plane ucsPlane = Utils.Db.GetUcsPlane(prRes.ObjectId.Database);
                    double rotAngle2 = entEcs.Xaxis.AngleOnPlane(ucsPlane);
                    ed.WriteMessage(string.Format("\nRotation angle relative to UCS: {0}",
                        Autodesk.AutoCAD.Runtime.Converter.AngleToString(rotAngle2, AngularUnitFormat.Current, -1)));
